...
def process_bruh_logic(df, start_num, end_num=0, max_jump=1500, filter_mode=1):
    ...
    for idx, item in enumerate(bruh_rows):
        ...
        debug_info = f"Targ:{current_target}|Auths:{recent_authors}|Verif:{is_verified}"
        ...
        if is_verified and found_num in target_to_line_map:
            ...
            # PURGE: Reset memory to ONLY the person before the mess and the fixer
            ...
            # Mark mistakes in the abandoned timeline as Fixed
        ...
        if found_num == current_target:
            if author in recent_authors:
                all_mistakes.append({
                    "Line": i, "Author": author, "Msg": msg, 
                    "Reason": "2-Person Rule", "Status": "Active", "Debug": debug_info
                })
        ...
        # --- STEP 4: TIMELINE BREAK (JUMP/ROLLBACK) ---
            ...
            all_mistakes.append({
                "Line": i, "Author": author, "Msg": msg, 
                "Reason": f"{reason} ({diff:+})", "Status": "Active", "Debug": debug_info
            })
            all_successes.append({"Line": i, "Author": author, "Msg": msg, "Status": "CORRECT"})
            
            last_valid_num, current_target = found_num, found_num + 1
            # Start fresh memory for the jump timeline
            recent_authors = [author]
            continue
