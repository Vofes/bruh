...
def process_bruh_logic(df, start_num, end_num=0, max_jump=1500, hide_invalid=False):
    ...
    bruh_rows = []
    for i, row in df.iterrows():import pandas as pd
import re

def process_bruh_logic(df, start_num, end_num=0, max_jump=1500, hide_invalid=False):
    pattern = re.compile(r'(?i)^bruh\s+(\d+)')
    cols_m = ["Line", "Author", "Msg", "Reason", "Status"] # Added Status
    cols_s = ["Line", "Author", "Msg", "Status"]
    ...
    for idx, item in enumerate(bruh_rows):
        ...
        # --- 3. VERIFIED JUMP / ROLLBACK (The Healing Engine) ---
        elif is_verified and abs(diff) <= max_jump:
            base_reason = "Jump" if diff > 0 else "Rollback"
            final_reason = f"{base_reason} ({diff:+})"
            
            # THE HEALER: Look back for a mistake to resolve
            fix_target_line = None
            
            # Check for the fixing rules within the window
            for m in reversed(all_mistakes):
                if m["Line"] < i - FIX_WINDOW: break # Stop if beyond window
                if m["Status"] != "Active": continue # Don't fix already fixed things
                ...
                # Rule Check
                is_fixing_rollback = (diff < 0 and ("Jump" in m["Reason"] or "2-Person" in m["Reason"]) and abs(found_num - m_val) <= 2)
                is_fixing_jump = (diff > 0 and "Rollback" in m["Reason"] and "Fixing" not in m["Reason"] and abs(found_num - m_val) <= 2)

                if is_fixing_rollback or is_fixing_jump:
                    final_reason = f"Fixing {base_reason} ({diff:+})"
                    fix_target_line = m["Line"]
                    break

            # If a fix was detected, PERFORM THE SWEEP
            if fix_target_line is not None:
                for mistake in all_mistakes:
                    if fix_target_line <= mistake["Line"] < i:
                        mistake["Status"] = f"Fixed (by {i})"
            
            # Log the current action and update chain
