    ...
    for idx, item in enumerate(bruh_rows):
        ...
        if not active_status:
            if found_num == start_num:
                ...
                last_valid_author, recent_authors = author, [author]
        ...
        # --- PRIORITY 1: THE FIXER (The Timeline Healer) ---
        if is_verified and found_num in target_to_line_map:
            origin_line = target_to_line_map[found_num]
            
            # HARD RESET: We force the list to ONLY contain the person from before the jump and the fixer.
            # This wipes _wynaut_ out of existence before 333401 is processed.
            recent_authors = [last_valid_author, author] if last_valid_author else [author]
            
            label = "Fixer (RB)" if diff < 0 else "Fixer (JP)"
            all_mistakes.append({"Line": i, "Author": author, "Msg": msg, "Reason": f"{label} ({diff:+})", "Status": "Fixer", "Debug": f"RESET_TO:{recent_authors}"})
        ...
        # --- PRIORITY 2: REPETITIONS & SWAPS (The Restricted Zone) ---
        if found_num == last_valid_num:
            fixed_via_swap = False
            # ONLY look back at the last 3 rows. No more "Time Travel" fixes.
        ...
        # --- PRIORITY 3: TARGET MATCH (The Normal Flow) ---
        ...
        if is_verified:
            ...
            all_mistakes.append({"Line": i, "Author": author, "Msg": msg, "Reason": f"{label} ({diff:+})", "Status": "Active", "Debug": "BREAK"})
            all_successes.append({"Line": i, "Author": author, "Msg": msg, "Status": "CORRECT"})
            last_valid_num, current_target = found_num, found_num + 1
            recent_authors = [author] # Start fresh on the jump
            continue
